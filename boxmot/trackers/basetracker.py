...
class BaseTracker(ABC):
    ...
    def plot_only_box_on_img(self, img: np.ndarray, box: tuple, conf: float, cls: int, id: int) -> np.ndarray:
        """
        Draws a bounding box with ID, confidence, and class information on an image.
    ...
    def plot_results_2(self, img: np.ndarray, show_trajectories: bool) -> np.ndarray:
        """
        Visualizes the trajectories of all active tracks on the image. For each track,
        it draws the latest bounding box and the path of movement if the history of
        observations is longer than two. This helps in understanding the movement patterns
    ...
    def plot_0_results(self, img: np.ndarray, show_trajectories: bool) -> np.ndarray:
        """
        Visualizes the trajectories of all active tracks on the image. For each track,
        it draws the latest bounding box and the path of movement if the history of
        observations is longer than two. This helps in understanding the movement patterns
            ...
            for k in self.per_class_active_tracks.keys():
                ...
                for a in active_tracks:
                    if a.history_observations:
                        if len(a.history_observations) > 2:
                            box = a.history_observations[-1]
                            img = self.plot_only_box_on_img(img, box, a.conf, a.cls, 0)
                            if show_trajectories:
                                img = self.plot_trackers_trajectories(img, a.history_observations, a.id)
        else:
            for a in self.active_tracks:
    ...
    def plot_lost_results(self, img: np.ndarray, show_trajectories: bool) -> np.ndarray:
        """
        Visualizes the trajectories of all active tracks on the image. For each track,
        it draws the latest bounding box and the path of movement if the history of
        observations is longer than two. This helps in understanding the movement patterns
                ...
                for a in active_tracks:
                    if a.history_observations:
                        if len(a.history_observations) > 2:
                            ...
                            if show_trajectories:
                                img = self.plot_trackers_trajectories(img, a.history_observations, a.id)
                # for b in self.lost_stracks:
                #     if b.history_observations:                    
                #         box = b.history_observations[-1]
                #         img = self.plot_box_on_img(img, box, b.conf, b.cls, b.id)
                #         if show_trajectories:
                #             img = self.plot_trackers_trajectories(img, b.history_observations, b.id)
    ...
    def save_txts(self, frame_id) -> np.ndarray:
        """
        Visualizes the trajectories of all active tracks on the image. For each track,
        it draws the latest bounding box and the path of movement if the history of
        observations is longer than two. This helps in understanding the movement patterns
                ...
                if a.history_observations:
                    if len(a.history_observations) > 2:
                        ...
                        #tracking_results =([
                            ...
                            box[0],                                 # bbox_left
                            box[1],                                 # bbox_top
                            abs(box[2] - box[0]),                   # bbox_width
                            abs(box[3] - box[1]),                   # bbox_height
                            a.conf,
                            -1,
                            -1,
                            -1
    ...
    def save_txts_lost(self, frame_id) -> np.ndarray:
        """
        Visualizes the trajectories of all active tracks on the image. For each track,
        it draws the latest bounding box and the path of movement if the history of
        observations is longer than two. This helps in understanding the movement patterns
    ...
    def save_txts_unmove(self, frame_id) -> np.ndarray:
        """
        Visualizes the trajectories of all active tracks on the image. For each track,
        it draws the latest bounding box and the path of movement if the history of
        observations is longer than two. This helps in understanding the movement patterns
