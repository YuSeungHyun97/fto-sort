...
class BYTETracker(BaseTracker):
    def __init__(
        self,
        first_thresh=0.5,# threshold for the first association
        second_thresh=0.1,# threshold for the second association
        match_thresh=0.8, # threshold for matching tracks
        new_track_thresh = 0.6, # threshold for init new track if the detection does not match any tracks
        track_buffer=25, # buffer to calculate the time when to remove tracks
        frame_rate=30,
        per_class=False,
    ):
        super().__init__()
    ...
    def update(self, dets: np.ndarray, img: np.ndarray = None, embs: np.ndarray = None) -> np.ndarray:
        ...
        remain_inds = confs > self.first_thresh

        inds_low = confs > 0.1
        inds_high = confs < self.first_thresh
        inds_low = confs > self.second_thresh
