...
from boxmot.utils.iou import iou_batch, diou_batch, giou_batch, centroid_batch, biou_batch, ciou_batch
...
#     # 임계값을 넘는 요소들을 매우 큰 값으로 설정 (매칭에서 제외)
#     high_value = cost_matrix.max() * 10  # 적당히 큰 값을 설정합니다.
#     cost_matrix[cost_matrix > thresh] = high_value

#     # 헝가리안 알고리즘을 적용하여 최적의 매칭을 찾음
#     from scipy.optimize import linear_sum_assignment
#     row_ind, col_ind = linear_sum_assignment(cost_matrix)

#     matches = []
#     for i, j in zip(row_ind, col_ind):
#         if cost_matrix[i, j] < high_value:  # 유효한 매칭만 추가
#             matches.append([i, j])
...
def iou_distance(atracks, btracks):
    ...
    if ious.size == 0:
        return ious
    #_ious = centroid_batch(atlbrs, btlbrs)
    #_ious = diou_batch(atlbrs, btlbrs)    
    #_ious = biou_batch(atlbrs, btlbrs)
...
def biou_distance(atracks, btracks):
    """
    Compute cost based on IoU
    :type atracks: list[STrack]
    :type btracks: list[STrack]
