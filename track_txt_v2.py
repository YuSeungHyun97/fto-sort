...
output_file_1 = "../TrackEval-master/data/trackers/mot_challenge/jochiwon-5M/2M30S_01/data/2M30S_01.txt"
output_file_2 = "../TrackEval-master/data/trackers/mot_challenge/jochiwon-5M/2M30S_01/data/2M30S_02.txt"
...
with open(output_file_1, 'w') as file1, open(output_file_2, 'w') as file2:
    for results in tracking_results:
        for result in results:
            frame_id = int(result[0])  # Assuming frame_id is the first element in result
            if frame_id <= 1260:
                file1.write(','.join(map(str, result)) + '\n')
            else:
                new_frame_id = frame_id - 1260
                result[0] = new_frame_id
