...
for i, img_file in enumerate(sorted(image_files)):
    ...
    if lost_tracking_temp:
        ...
        # print("lost_tracking_results[0] :", lost_tracking_results[0])
        # print("lost_tracking_results[0][0][0] :", lost_tracking_results[0][0][0])

    print("lost_tracking_results :", lost_tracking_results)
    #if lost_tracking_results:
    for item in lost_tracking_results[:]:
        #if (a[0][0][0] == (frame_id - 3)):
        if item[0][0] == (frame_id - 3):
            #tracking_results.append(item)
            print("item :", item)
            lost_tracking_results.remove(item)
    print("\nAfter_lost_tracking_results :", lost_tracking_results)
